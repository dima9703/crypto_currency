...
void CryptoData::Initialize(json input_config) {
  ...
  for (size_t i = 0; i < max_files_number; i++) {
    std::string path =
        "../data/" + files[i] + "/day/" + files[i] + "_day_1.csv";
  ...
  size_t last_element_index = (size_t) timestamps_vec.size() * percentage_data / 100;
  ...
  matrix timestamps_vec_to_csv;
    ...
    for (size_t iter = 0; iter < timestamps_vec.size(); ++iter) {
  ...
  for (uint64_t i = 0; i < all_data.size(); i++) {
    ...
    for (uint64_t j = 0; j < all_data[i].size(); j++) {
        std::vector<int64_t> input_necessary_columns;
        for(size_t index = 0; index < necessary_columns.size(); index++) {
            input_necessary_columns.push_back(all_data[i][j]necessary_columns[index]);
        }
      ab.insert(std::pair<int64_t, std::vector<double>>(
          all_data[i][j][0],
          input_necessary_columns));
    }
  ...
  for (uint64_t iter = 0; iter < timestamps_vec.size(); iter++) {
    ...
    for (uint64_t jiter = 0; jiter < map_tables.size(); jiter++) {
      ...
      if (search != map_tables[jiter].end()) {
        ...
        sub_current_array_exists = {1, 1, 1, 1, 1, 1};
      } else {
        sub_current_array = {0, 0, 0, 0, 0, 0};
        sub_current_array_exists = {0, 0, 0, 0, 0, 0};
      }
  ...
  matrix final_matrix_a(final_matrix.begin(), final_matrix.begin() + last_element_index);
  mask exists_element_a(exists_element.begin(),exists_element.begin() + last_element_index);
...
int64_t CryptoData::calculate() { return CryptoData::qwer; }
